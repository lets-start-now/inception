...
export default function ContributionGraph() {
  ...
  const [cursor,    setCursor]    = useState(() => startOfMonth(new Date()))  // mobile month view
  const scrollRef = useRef(null)
  const detailRef = useRef(null)

  useEffect(() => { if (profile) loadData() }, [profile])

  // Start scrolled to the most recent weeks (today) instead of a year ago.
  useEffect(() => {
  ...
  // On mobile, tapping a day should reveal its detail card (rendered below).
  useEffect(() => {
  ...
  async function loadData() {
    ...
    const { data } = await supabase
      ...
      .eq('user_id', profile.id).gte('summary_date', since).order('summary_date')
  ...
  // Mobile month navigation bounds: from the first month with activity → this month.
      ...
      {loading ? <LoadingSpinner /> : (
        <>
          <div className="grid grid-cols-3 gap-4">
            <GraphStat icon={CheckCircle2} tint="emerald" value={stats.completed} label="Days completed" />
            <GraphStat icon={Crown}        tint="amber"   value={stats.leader}    label="Leader days" />
            <GraphStat icon={Flame}        tint="amber"   value={`${stats.streak}d`} label="Current streak" />
          </div>
...
function MonthGrid({ monthDate, summaries, cellColor, cellTitle, onSelect }) {
  ...
  const pad   = (getDay(start) + 6) % 7   // Monday-based leading blanks
...
function GraphStat({ icon: Icon, tint, value, label }) {
  return (
